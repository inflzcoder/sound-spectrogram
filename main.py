...
class MouseCreate:
	def __init__(self):
		self.name = """LEFT MOUSE - GIVE VELOCITY (HOLD AND DRAG)
RIGHT MOUSE - CHANGE BODY RADIUS
LEFT MOUSE - CREATE BODY (CLICK)
WASD - MOVE IN SPACE
	...
	def render(self):
		...
		if right_dragging:
			...
			for tick in range(500):
				...
				for obj in _objs:
					px.pset(obj.x-camera_x, obj.y-camera_y, obj.color)
		else:
...
class MouseBlackHole:
	...
	def right_dragging(self):
		...
		for d in range(a):
			angle = (d/a)*2*pi
			px.pset(cos(angle)*self.radius+40, sin(angle)*self.radius+40, 7)
...
def update():
	...
	if px.btnp(px.MOUSE_BUTTON_MIDDLE):
